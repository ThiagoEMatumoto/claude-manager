...
const EMPTY_LABEL: Record<CcTab, string> = {
  plugins: 'Nenhum plugin instalado ou habilitado.',
...
export function CcConfigsView({ tab, configs, loading }: Props) {
  ...
  if (items.length === 0) {
    return <CenterMessage text={EMPTY_LABEL[tab]} />
  }
  ...
  return (
    <div className="h-full overflow-y-auto px-4 py-4">
      <ul className="flex flex-col gap-px">
        {tab === 'plugins' &&
          configs.plugins.map((p) => <PluginRow key={`${p.name}@${p.marketplace}`} plugin={p} />)}
...
function PluginRow({ plugin }: { plugin: PluginInfo }) {
  return (
    <RowShell
      name={plugin.name}
      meta={plugin.marketplace || undefined}
      badge={
        <span
