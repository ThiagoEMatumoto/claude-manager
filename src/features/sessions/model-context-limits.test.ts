...
describe('contextUsage', () => {
  ...
  it('uses the 1M limit for a fable id', () => {
    const u = contextUsage({ tokens: { output: 0, context: 100_000 }, model: 'claude-fable-5' })
    expect(u).toEqual({ used: 100_000, limit: 1_000_000, pct: 10 })
  })
...
describe('modelSupportsXhigh', () => {
  it('fable, opus and sonnet support xhigh', () => {
    expect(modelSupportsXhigh('claude-fable-5')).toBe(true)
    expect(modelSupportsXhigh('claude-opus-4-8')).toBe(true)
    expect(modelSupportsXhigh('claude-sonnet-4-5')).toBe(true)
  })

  it('haiku does not support xhigh', () => {
    expect(modelSupportsXhigh('claude-haiku-4-5')).toBe(false)
  })

  it('unknown or missing model → false (fail-closed)', () => {
    expect(modelSupportsXhigh('gpt-something')).toBe(false)
