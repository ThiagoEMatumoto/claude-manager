...
interface Props {
  ...
  onCycle?: () => void
  /** Painel estreito: troca pro label curto e esconde o chevron — o valor
   * continua sempre visível, nunca escondido atrás de um menu. */
  compact?: boolean
}
...
export function PermissionPill({ currentMode, onSelect, onCycle, compact }: Props) {
  const [open, setOpen] = useState(false)
  const style = permissionStyle(currentMode)
  const activeLabel = compact
    ? (currentMode ? PERMISSION_SHORT_LABELS[currentMode] : PERMISSION_SHORT_LABELS.default)
  ...
  return (
    <Menu open={open} onClose={() => setOpen(false)} sections={sections} portal align="left">
      <button
        ...
        className={`flex items-center gap-1 rounded-full border border-[var(--color-border)] py-0.5 text-[10px] transition hover:border-current/50 ${
          compact ? 'px-1.5' : 'px-2'
        } ${style.text}`}
      >
